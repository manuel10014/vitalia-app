...
import { Asset, Protocol, ProtocolField } from '@/types';
...
export function RunTestDialog() {
  ...
  // Casteamos fields para asegurar que TypeScript sepa que es un array de ProtocolField
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader><DialogTitle>Nuevo Reporte Técnico</DialogTitle></DialogHeader>
        <div className="grid gap-4 py-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Select onValueChange={setSelectedAssetId} value={selectedAssetId}>
                ...
                <SelectContent>
                  {assets?.map(a => <SelectItem key={a.id} value={a.id}>{a.tagId}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
              <Label>Protocolo</Label>
              <Select onValueChange={setSelectedProtocolId} value={selectedProtocolId}>
                <SelectTrigger><SelectValue placeholder="Seleccionar..." /></SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-4 bg-gray-50 p-4 rounded-md border">
              {protocolFields.map((field) => (
                <div key={field.id} className="grid grid-cols-4 items-center gap-4">
                  ...
                  {field.type === 'text' && <Input className="col-span-3" onChange={e => handleInputChange(field.label, e.target.value)} />}
                  {field.type === 'number' && <Input type="number" className="col-span-3" onChange={e => handleInputChange(field.label, Number(e.target.value))} />}
                  {field.type === 'check' && (
                    <Select onValueChange={val => handleInputChange(field.label, val)}>
          ...
          ) : <div className="text-center py-4 text-gray-400">Selecciona un protocolo.</div>}
          <Button onClick={() => mutation.mutate()} disabled={mutation.isPending} className="w-full mt-4">Guardar Reporte</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
